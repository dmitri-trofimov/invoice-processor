...
function processInvoice(invoice: Invoice): ProcessedInvoice | null {
    if (!isHappyWriterInvoice(invoice)) {
        return null;
    }
    ...
    const processedFileName = `${invoiceDate} — Invoice ${invoiceNumber} — Happy Writer (${service}).pdf`;
...
export const happyWriterInvoiceProcessor = processInvoice;

function isHappyWriterInvoice(invoice: Invoice): boolean {
    return invoice.extractedText.includes("Happy Writer OÜ") && invoice.extractedText.includes("Registrikood 14000245");
}
