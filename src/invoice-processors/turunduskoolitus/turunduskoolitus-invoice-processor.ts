...
function processInvoice(invoice: Invoice): ProcessedInvoice | null {
    if (!isTurunduskoolitusInvoice(invoice)) {
        return null;
    }
    ...
    const processedFileName = `${invoiceDate} — Invoice ${invoiceNumber} — Turunduskoolitus (${service}).pdf`;

    const processedInvoice: ProcessedInvoice = {
        ...invoice,
        processorName: "Turunduskoolitus",
        processedFileName,
    };
...
export const turunduskoolitusInvoiceProcessor = processInvoice;

function isTurunduskoolitusInvoice(invoice: Invoice): boolean {
    return invoice.extractedText.includes("14147132") && invoice.extractedText.includes("EE101952617");
}
